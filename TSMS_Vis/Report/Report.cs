...
namespace TSMS_Vis.Report
{
    public class Report
    {
        ...
        public Report(ulong lot_nr, ReportType reporttype)
        {
            if (reporttype.CompleteLOT)
                _reportfilename = lot_nr.ToString();
            else
                _reportfilename = lot_nr.ToString() + "_from_" + reporttype.Section.StartPart;

            Mouse.OverrideCursor = System.Windows.Input.Cursors.Wait;       

            DbHandler db = new DbHandler();
            var reportData = db.CollectReportData(lot_nr, reporttype);
        ...
        public void WriteToPdf(string path)
        {
            try
            {
                var filename = path + @"\" + String.Format("{0}.pdf", _reportfilename);
