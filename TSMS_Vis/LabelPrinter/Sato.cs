...
namespace TSMS_Vis
{
    //  Tested with Sato CL4NX
    public class Sato : LabelPrinterInterface
    {
        private bool Connect(ref TcpClient tcp, string ipAddress, int port)
        {
            ...
            if (tcp.GetStream().Read(status, 0, status.Length) > 0 && status[3] == 28)
            {
                Logger.Write("SATO printer initialized");
            }
            else
                return false;
